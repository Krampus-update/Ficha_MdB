...
local function constructNew_frmContador()
    ...
    obj.cbBarChoose:setField("cbBarChoose");

    obj.dataLink7 = GUI.fromHandle(_obj_newObject("dataLink"));
    obj.dataLink7:setParent(obj.layout10);
    obj.dataLink7:setFields({'cbBarChoose','valMax','valCur'});
    ...
    obj._e_event7 = obj.dataLink7:addEventListener("onChange",
        function (_, field, oldValue, newValue)
            Dialogs.confirmYesNo(valCur .. "/" .. valMax.."?",function (confirmado)
            										if confirmado then
            											enviar(valCur.."/"..valMax)
            										end;
            									 end);
        end, obj);
